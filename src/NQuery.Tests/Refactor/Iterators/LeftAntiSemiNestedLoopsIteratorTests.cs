...
namespace NQuery.Tests.Refactor.Iterators
{
    public class LeftAntiSemiNestedLoopsIteratorTests : IteratorTests
    {
        ...
        public void Iterators_LeftAntiSemiNestedLoops_ForwardsProperly()
        {
            ...
            using (var iterator = new LeftAntiSemiNestedLoopsIterator(left, right, _ => true, _ => false))
            {
                for (var i = 0; i < 2; i++)
                {
        ...
        public void Iterators_LeftAntiSemiNestedLoops_NoReadsOnRight_WhenPassthruIsTrue()
        {
            ...
            using (var iterator = new LeftAntiSemiNestedLoopsIterator(left, right, predicate, passthru))
            {
        ...
        public void Iterators_LeftAntiSemiNestedLoops_ReturnsLeft_WhenRightIsNonEmpty_UnlessPassthruIsTrue()
        {
            ...
            using var iterator = new LeftAntiSemiNestedLoopsIterator(left, right, _ => true, passthru);
            AssertProduces(iterator, expected);
        }
        ...
        public void Iterators_LeftAntiSemiNestedLoops_MatchesRows()
        {
            ...
            using var iterator = new LeftAntiSemiNestedLoopsIterator(left, right, predicate, _ => false);
            AssertProduces(iterator, expected);
        }
    }
}
