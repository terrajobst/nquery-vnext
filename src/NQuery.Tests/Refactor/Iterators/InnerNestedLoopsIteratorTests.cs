...
{
    // Predicate and passthru receive the combined (left ++ right) buffer: left column i
    // is rb[i], right column j is rb[leftColumnCount + j].
    public class InnerNestedLoopsIteratorTests : IteratorTests
    {
        ...
        public void Iterators_InnerNestedLoops_ForwardsProperly()
        {
            ...
            using (var iterator = new InnerNestedLoopsIterator(left, right, _ => true, _ => false))
            {
                for (var i = 0; i < 2; i++)
                {
        ...
        public void Iterators_InnerNestedLoops_NoReadsOnRight_WhenPassthruIsTrue()
        {
            ...
            using (var iterator = new InnerNestedLoopsIterator(left, right, predicate, passthru))
            {
                AssertProduces(iterator, expected);
            }
        ...
        public void Iterators_InnerNestedLoops_ReturnsEmpty_WhenRightIsEmpty_UnlessPassthruIsTrue()
        {
            ...
            using var left = new MockedIterator(leftRows);
            using var right = new MockedIterator(rightRows);
            var passthru = new EmittedPredicate(rb => Equals(rb[0], 2));

            using var iterator = new InnerNestedLoopsIterator(left, right, _ => true, passthru);
            AssertProduces(iterator, expected);
        }
        ...
        public void Iterators_InnerNestedLoops_MatchesRows()
        {
            ...
            using var left = new MockedIterator(leftRows);
            using var right = new MockedIterator(rightRows);
            // left columns are rb[0..1], right columns rb[2..3]; match on the leading key.
            var predicate = new EmittedPredicate(rb => Equals(rb[0], rb[2]));

            using var iterator = new InnerNestedLoopsIterator(left, right, predicate, _ => false);
            AssertProduces(iterator, expected);
        }
    }
}
