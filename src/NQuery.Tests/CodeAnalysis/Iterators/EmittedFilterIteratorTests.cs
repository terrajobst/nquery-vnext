...
public class EmittedFilterIteratorTests : IteratorTests
{
    [Fact]
    public void Iterators_EmittedFilter_ForwardsProperly()
    {
        var rows = new object[] { 1, 2 };
        var expected = rows;

        using var input = new MockedIterator(rows);
        using (var iterator = new EmittedFilterIterator(input, _ => true, outer: null))
        {
            for (var i = 0; i < 2; i++)
            {
    ...
    [Fact]
    public void Iterators_EmittedFilter_ReturnsEmpty_IfInputIsEmpty()
    {
    ...
    [Fact]
    public void Iterators_EmittedFilter_ReturnsEmpty_IfPredicateIsFalse()
    {
        using var input = new ConstantIterator();
        using var iterator = new EmittedFilterIterator(input, _ => false, outer: null);
        AssertEmpty(iterator);
    }

    [Fact]
    public void Iterators_EmittedFilter_Filters()
    {
