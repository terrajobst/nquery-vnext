...
// Compiles a whole stream-aggregate's folds into a factory for CompiledAggregates. Each fold's
...
internal static class AggregateCompiler
{
    public static Func<CompiledAggregates> Compile(ImmutableArray<LogicalAggregatedValue> aggregates, FrozenDictionary<ValueSlot, RowBufferColumn> slotIndices, ImmutableArray<RowBufferColumn> outputColumns, ImmutableArray<Type> outputTypes)
    {
        ...
        for (var i = 0; i < aggregates.Length; i++)
        {
            ...
            var argument = ExpressionCompiler.CompileFunction(aggregates[i].Argument, slotIndices);
            ...
            storeStatements[i] = ExpressionCompiler.BuildWriteCall(target, outputColumns[i], slotType, result);
        }
        ...
        var factory = Expression.Lambda<Func<CompiledAggregates>>(
            Expression.Block(
                states,
                Expression.New(
                    typeof(CompiledAggregates).GetConstructors()[0],
