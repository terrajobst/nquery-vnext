...
The **Parser** (`NQuery.Syntax.Parser`) is a hand-written recursive-descent parser that consumes tokens from the `Lexer` and produces a `SyntaxTree`. It parses the following grammar structure:
